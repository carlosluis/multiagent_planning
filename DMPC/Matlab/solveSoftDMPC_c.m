function [p,v,a,success] = solveSoftDMPC_c(po,pf,vo,ao,n,h,l,K,rmin,pmin,pmax,alim,A,A_initp,Delta,Q1,S1,E1,E2,order)
...
if (violation) % In case of collisions, we relax the constraint with slack variable
    ...
    f_eps = -5*10^3*(K/k)^2*[zeros(3*K,1); ones(N-1,1)]';
    
    % Quadratic penalty on collision constraint relaxation
    EPS = 5*10^3*(K/k)^2*[zeros(3*K,3*K) zeros(3*K,N-1);
